...
namespace AirManager.Services.Database
{
    ...
    {
        // ✅ DINAMICO:  Legge da StationRegistry l'emittente attiva
        private static string DatabasePath
        {
            get
            {
                try
                {
                    string path = AirManager.Services.StationRegistry.GetCurrentDatabasePath();

                    if (string.IsNullOrEmpty(path))
                    {
                        Console.WriteLine("[DbcManager] ⚠️ Nessun database path configurato, uso fallback");
                ...
                {
                    Console.WriteLine($"[DbcManager] ❌ Errore lettura DatabasePath: {ex.Message}");
                    return @"C:\AirDirector\Database";
                }
            }
        }
        ...
        /// <summary>
        /// Inizializza il database (crea cartella e file di default se non esistono)
        /// </summary>
        public static void Initialize()
        {
            try
            {
                ...
                Console.WriteLine($"[DbcManager] Inizializzazione database: {dbPath}");
        ...
        /// <summary>
        /// Crea i file .dbc di default se non esistono
        /// </summary>
        private static void CreateDefaultFiles()
        {
            ...
            if (!File.Exists(configPath))
            {
                var defaultConfig = new List<ConfigEntry>
                {
                    ...
                    new ConfigEntry { Key = "BackupPath", Value = Path.Combine(DatabasePath, "Backups") },
            ...
            // ✅ REPORT.DBC
            string reportPath = Path.Combine(DatabasePath, "Report.dbc");
            if (!File.Exists(reportPath))
            {
                ...
                Console.WriteLine("[DbcManager] ✅ Report.dbc creato");
            }
        }
        ...
        /// <summary>
        /// Salvataggio interno (senza lock, usato da CreateDefaultFiles)
        /// </summary>
        ...
        public static bool Update<T>(string fileName, T entry) where T : class
        {
            lock (_lock)
            {
                try
                {
                    ...
                    Console.WriteLine($"[DbcManager] Entry totali caricate: {allEntries.Count}");
                    ...
                    Console.WriteLine($"[DbcManager] Risultato salvataggio: {success}");
                    Console.WriteLine($"[DbcManager] ====================================\n");
        ...
        /// <summary>
        /// Verifica se il file è stato modificato dall'ultimo caricamento
        /// </summary>
        ...
        /// <summary>
        /// Ottiene il path completo di un file .dbc
        /// </summary>
        ...
        /// <summary>
        /// Ottiene il Database Path corrente
        /// </summary>
    ...
    public class EncoderEntry : IDbcEntry
    {
        public int ID { get; set; }
        public string Name { get; set; } = "";
        public string Type { get; set; } = "";
