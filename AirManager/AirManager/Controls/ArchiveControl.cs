...
namespace AirManager.Controls
{
    public partial class ArchiveControl : UserControl
    {
        ...
        public ArchiveControl(string archiveType)
        {
            ...
            // DailyLogger not available in AirManager
            InitializeUI();
        ...
        private void DgvArchive_MouseDown(object sender, MouseEventArgs e)
{
    ...
    if (e.Button == MouseButtons.Left)
    {
        ...
        {
            bool isRadioTVMode = (StationRegistry.GetActiveStation()?.StationType == StationType.RadioTV);
            ...
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                if (audioFiles.Length > 0)
                    OpenVideoConvertionForm(audioFiles);

                // video files → open conversion form (non-modal) – only in RadioTV mode
                if (videoFiles.Length > 0)
                {
                    if (isRadioTVMode)
                        OpenVideoConvertionForm(videoFiles);
                    else
        ...
        // ─────────────────────────────────────────────────────────────────────────────
        // NEW METHOD:  OpenVideoConvertionForm
        // ─────────────────────────────────────────────────────────────────────────────
        private void OpenVideoConvertionForm(string[] videoFiles)
        {
            var convForm = new VideoConvertionForm(videoFiles, _archiveType);

            // When conversion finishes, import the resulting mp4 files into the archive.
            // This callback is raised on the UI thread (see VideoConvertionForm).
        ...
        //  conversion form — all video handling now routes through VideoConvertionForm,
        //  so here we can always treat .mp4 as a regular video import)
        // ─────────────────────────────────────────────────────────────────────────────
        private void ImportFiles(string[] filePaths, VideoConvertionForm convForm = null)
        {
        ...
        private void Log(string m) { Console.WriteLine($"[ArchiveControl] {m}"); }
        private void LogErr(string m, Exception ex) { Console.Error.WriteLine($"[ArchiveControl] {m}: {ex}"); }
        private void LogErr(string m) { Console.Error.WriteLine($"[ArchiveControl] {m}"); }
