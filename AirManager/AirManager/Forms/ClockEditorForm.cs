    ...
    public partial class ClockEditorForm : Form
    {
        ...
        private void ApplyLanguage()
        {
            ...
            if (lblItemsTitle != null)
                lblItemsTitle.Text = "📋 " + LanguageManager.GetString("ClockEditor.ElementsList",
                    "ELEMENTI DEL CLOCK (trascina per riordinare)");
        ...
        private void UpdateFilterCount()
        {
            try
            {
                ...
                if (radMusic != null && radMusic.Checked)
                {
                    var musicEntries = DbcManager.LoadFromCsv<MusicEntry>("Music.dbc");
                ...
                if (lblFilterCount == null) return;

                if (count > 0)
                {
                    TimeSpan avgTime = TimeSpan.FromSeconds(avgDuration);
                    lblFilterCount.Text = string.Format(
                        LanguageManager.GetString("ClockEditor.FoundTracks", "📊 Trovati: {0} brani\n\nDurata media: {1}"),
                        count,
        ...
        private void RefreshClockItemsList()
        {
            ...
            if (lblTotalDuration != null)
            {
                ...
                lblTotalDuration.Text = string.Format(
                    LanguageManager.GetString("ClockEditor.TotalDuration", "⏱️ Durata Totale: {0}"),
        ...
        private Panel CreateClockItemCard(ClockItem item, int index, TimeSpan cumulativeTime)
        {
            ...
            {
                Text = $"[{cumulativeTime.ToString(@"hh\:mm\:ss")}]",
            ...
            btnDelete.Click += (s, e) =>
            {
                if (capturedIndex < _clockItems.Count)
                {
                    _clockItems.RemoveAt(capturedIndex);
                    RefreshClockItemsList();
                }
            };
        ...
        private string GetDetailedFilterInfo(ClockItem item, string sourceType)
        {
            ...
            if (item.YearFilterEnabled)
            {
                parts.Add(string.Format(
                    LanguageManager.GetString("ClockEditor.Years", "Anni: {0}-{1}"),
        ...
        {
            if (cmbCategory != null) cmbCategory.Enabled = chkFilterCategory.Checked;
            if (cmbGenre != null) cmbGenre.Enabled = chkFilterGenre.Checked;
            UpdateFilterCount();
        }
        ...
        private void FlowClockItems_DragDrop(object sender, DragEventArgs e)
        {
            ...
            for (int i = 0; i < flowClockItems.Controls.Count; i++)
            {
                ...
                if (clientPoint.Y < ctrl.Bottom && ctrl.Tag is int)
                {
