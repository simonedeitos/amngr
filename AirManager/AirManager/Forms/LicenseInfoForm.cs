...
namespace AirManager.Forms
{
    public partial class LicenseInfoForm : Form
    {
        ...
        private void BuildUI()
        {
            ...
            {
                Text = LanguageManager.GetString("LicenseInfo.Header", "Gestione Licenza"),
            ...
            AddCardRow(pnlCard, "🖥️ " + LanguageManager.GetString("LicenseInfo.MachineId", "ID Macchina"), machineId, 132);
            ...
            {
                Text = "🗑 " + LanguageManager.GetString("LicenseInfo.BtnRemove", "Rimuovi Licenza"),
        ...
        {
            var license = LicenseManager.GetCurrentLicense();
            string detail = license != null
                ? $"{license.OwnerName} — {license.SerialKey}"
                : "-";

            using (var confirmForm = new LicenseRemoveConfirmForm(detail))
            {
                if (confirmForm.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    {
                        MessageBox.Show(
                            LanguageManager.GetString("LicenseInfo.RemoveError", "Errore durante la rimozione: ") + error,
